...
fn grit() -> Dictionary {
    ...
    // 尝试注册 :GritHello 命令
    let hello_result =
        api::create_user_command("GritHello", "echo 'Hello World from Grit plugin!'", &hello_opts);
    ...
    let grit_result =
        api::create_user_command("Grit", "tabnew", &grit_opts);
